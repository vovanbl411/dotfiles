...
# Если окна не нашлись — выходим
if [ -z "$NVTOP_ID" ]; then exit 1; fi

# 4. САМЫЙ ВАЖНЫЙ ЭТАП: возвращаем контекст на левый монитор
# Сначала фокусируем сам воркспейс, чтобы действия применялись там
niri msg action focus-workspace "left"
sleep 0.2

# 5. Выполняем связку
# Мы фокусируем nvtop по ID и заставляем его "прилипнуть" к btop
niri msg action focus-window --id "$NVTOP_ID"
niri msg action consume-or-expel-window-left

# На всякий случай повторим фокусировку на btop, если хочешь, чтобы он был активным
